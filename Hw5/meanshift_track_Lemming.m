close all; 
clear all;
test_seq = 'Lemming';  % 选择视频的名字 'Lemming' 'Woman'
...
for frame = 2 : frame_number
    ...
    while((delta(1)^2 + delta(2)^2 > 0.5) && iter_num < 20)   %迭代条件,默认20次迭代
        ...
        for i = 1:a
            for j = 1:b
                ...
                hist2(q_temp(i,j)+1) = hist2(q_temp(i,j)+1) + m_wei(i,j);    % 计算直方图统计中每个像素点占的权重
            end
        end
        hist2 = hist2 * C; 
        ...
        for i = 1:a
            for j = 1:b
                sum_w = sum_w + w(uint32(q_temp(i,j))+1);
                xw = xw + w(uint32(q_temp(i,j))+1)*[i-y(1)-0.5,j-y(2)-0.5];
            end
        end
        Y = xw / sum_w;
    ...
    pic = getframe(gcf);
    img = frame2im(pic);
    imwrite(img,['images/', num2str(frame), '.jpg'],'jpg');
    hold off;
end
